...
const ParcelList = () => {
  ...
  const onItemPressed = (data) => {
    navigation.navigate('Carrier Parcels List', { data });
  };
  ...
  const renderItem = (param) => {
    ...
    return (
      <ListItem
        onItemPressed={() =>
          onItemPressed({ id: item.id.$oid, items: item.items, carrierId: item.carrier })
        }
        isDisabled={item.status === 'DELIVERED'}>
  ...
  return (
    <>
      <View style={headingStyles.container}>
        ...
        <Text style={headingStyles.title}>{`Parcel List ${data[0].pickupDate}`}</Text>
      </View>
      <Text style={headingStyles.subTitle}>
        {`${data.reduce((acc, curr) => acc + curr.itemsCount, 0)} items to be picked up`}
      </Text>
      <FlatList data={data} renderItem={renderItem} keyExtractor={(item) => item.id.$oid} />
    </>
  );
};
